...
public class ViewLoaderServiceDetails implements Initializable {
    ...
    @FXML
    private TableView<LoadService> loadService;

    @FXML
    private TableColumn<LoadService, String> L_id;

    @FXML
    private TableColumn<LoadService, String> L_description;

    @FXML
    private TableColumn<LoadService, String> L_date;

    @FXML
    private TableColumn<LoadService, String> L_price;
    ...
    public void initialize(){
        ...
        // get collection
        LoaderCollection = database.getCollection("LoaderServiceDetails");
    }
    ...
    public void showCrusher() {
        ...
        ObservableList<LoadService> list = getCrusherList();

        L_id.setCellValueFactory(data -> new SimpleStringProperty(data.getValue().getId()));
        L_description.setCellValueFactory(new PropertyValueFactory<LoadService, String>("Name"));
        L_price.setCellValueFactory(new PropertyValueFactory<LoadService, String>("Price"));
        L_date.setCellValueFactory(new PropertyValueFactory<LoadService, String>("Date"));


        loadService.setItems(list);

    }

    private ObservableList<LoadService> getCrusherList() {
        ObservableList<LoadService> attend = FXCollections.observableArrayList();
        ...
        try {
            ...
            for (int i = 0; i < LoaderCollection.count(); i++) {
                ...
                attend.add(new LoadService(loadid, loadname, loadpri, loaddate));
...
//          close the connection
    ...
    void handleMouseAction(MouseEvent event) {
        LoadService vehicle = loadService.getSelectionModel().getSelectedItem();
